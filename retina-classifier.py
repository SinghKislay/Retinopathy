...
def data_pipe_line(data,checkpoint_path,i_data=None,epoch=200):
    ...
    placeholder_X=tf.placeholder(tf.float32,shape=[None,500,400,3])
    placeholder_Y=tf.placeholder(tf.uint8,shape=[None])
    placeholder_X=next_image
    placeholder_Y=next_label
    Y=tf.one_hot(placeholder_Y,5)
    Y=tf.cast(Y,tf.float32)
    logits=model(placeholder_X)
...
def model(in_data):
    ...
    fc3 = tf.layers.dense(fc1, 256,activation=tf.nn.relu)
    fc3 = tf.layers.dense(fc1, 256,activation=tf.nn.relu)
